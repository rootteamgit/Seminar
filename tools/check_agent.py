...
[ミス #1] 台本とスライドの枚数ズレ
  発生：第21回台本生成時
  原因：スライド枚数を確認せずに台本を生成した
  対策：check_script_slide_alignment()でS1〜Snの連続性を必ずチェック

[ミス #2] 台本内部タイトルのバージョン番号更新忘れ
  発生：複数回
  原因：ファイル名はv2に上げたが台本内の「台本（v1）」を直し忘れ
  対策：check_script_version_title()でファイル名と内部タイトルのvN一致を確認

[ミス #3] 企画書アジェンダを見ずに台本を書いた
  発生：第17回台本生成時（第13回素材をベースに書いてしまった）
  原因：企画書の構成を確認する前に台本生成を開始した
  ルール：台本生成前に必ず企画書のアジェンダをextract_agenda_from_xlsx()で取得すること

[ミス #4] 台本の文字数が足りず継ぎ足しループが発生した
  発生：第22・23回台本生成時
  原因：目標文字数を事前に計算せず、生成→測定→継ぎ足しを繰り返した
  対策：calculate_target_chars()で事前設計→check_section_lengths()で事後一括チェック

[ミス #5] Google Drive read_file_contentがgid（シート）指定に対応していない
  発生：第23回企画書読み込み時
  原因：read_file_contentはgid指定を無視して全シートを返す
  対策：download_file_contentでxlsx全体を取得→openpyxlでシート名を直接指定して読む

[ミス #6] KPIファイルを原本と異なる項目構成で生成した
  発生：2026/7/1 KPI管理表作成時
  原因：原本（Google DriveのKPI管理表シート）の項目を確認せずに自分で項目を設計した
  対策：集計・KPIファイルを生成する前に必ず原本のヘッダー行をread_file_contentで取得し、
        項目・順序・単位を完全一致させること
  原本の正しい項目順：
    開催日 / セミナー名 / 登壇者 / 広告費 /
    集客（Meta・メルマガ・合計）/
...
[ミス #12] docxのp.textでセリフが「空」と誤判定した
  発生：第21回台本v7の羽生パート確認時
  原因：docxライブラリの`p.text`はrunが複数段落に分割されている場合に正しく結合されず、
        ラベル行の次の段落にセリフが入っていても「空」と誤報告する
  対策：docxファイルの内容確認・編集は必ずXMLレベルで行うこと
        1. unzip して word/document.xml を直接参照する
        2. re.sub(r'<[^>]+>', '', xml) でタグ除去してテキスト全文を確認する
        3. p.text が空でも「セリフがない」と判断しない
        4. 編集も unzip→XML直接操作→zip で行う
        ※ p.text が空 → 「セリフなし」と断言することは禁止

[ミス #7] 企画書シートのURLからgidを取得したがシートを特定できなかった
  発生：2026/7/1 第23回企画書読み込み時
  原因：read_file_contentにgidを渡しても無視される。複数回試みて時間を浪費した
  対策：gidを使ったシート特定はそもそも機能しない。
        download_file_content→openpyxlでシート名検索が唯一の正解。
        URLにgidが含まれていても即座にdownload_file_content方式に切り替えること

[ミス #8] ツール呼び出し上限に達して台本生成が途中で止まった
  発生：2026/7/1 第22回台本生成時
  原因：str_replaceを1セクションずつ小刻みに繰り返したため上限に到達
  対策：
    1. 事前にcalculate_target_chars()で各章の目標文字数を計算する
    2. 増量が必要な全セクション分のテキストをまとめて1回のstr_replaceで追記する
    3. 細かい確認は2〜3回の大きな編集ごとに行う（毎回確認しない）

[ミス #9] 台本生成後にcheck_agentを通さずに出力した
  発生：2026/7/1 第22・23回台本生成時
  原因：生成完了後そのままpresent_filesを呼んでしまった
  対策：台本生成後は必ずcheck_agentのローカルチェック（バージョン番号・スライド番号・文字数）を
        実行してからpresent_filesを呼ぶこと。チェックなしの出力は禁止

[ミス #10] outputsにコピーしただけでpresent_filesを呼ばずに完了と報告した
  発生：2026/7/1 check_agent.py更新時
  原因：cp コマンドで/mnt/user-data/outputs/にコピー後、present_filesを呼ぶのを忘れた
  対策：ファイルをemaさんに渡す際は必ずpresent_filesを呼ぶこと。
        「outputsにコピーした」だけでは相手はダウンロードできない
"""
